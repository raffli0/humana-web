...
export default function Navbar() {
    ...
    return (
        <div>
        <nav className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-black backdrop-blur-xl sticky top-0 z-50">
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href ? "font-semibold text-white" : "text-white/60"
              )}
            >
              {item.name}
            </Link>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-full bg-slate-900/70 px-2 py-1 ring-1 ring-slate-800">
